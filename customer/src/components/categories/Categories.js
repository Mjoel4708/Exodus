...
function Categories() {
    ...
    return (
        <Col fluid className="justify-content-center" style={{padding: "40px", color: "#000000"}}>
            {cats.map((cat) => (
                <Button toggle variant="light" style={{padding: "auto",  backgroundImage: `url(${cat.url})`, backgroundSize: 'cover', height: cat.height, width: cat.width, backgroundRepeat: "no-repeat", margin: "0px", fontFamily: "Oswald, sans-serif"}}><Button variant="outline-warning"><b>{cat.title}</b></Button></Button>
            ))}
        </Col>
